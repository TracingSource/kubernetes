...
func newControlPlanePrepareDownloadCertsSubphase() workflow.Phase {
	return workflow.Phase{
		...
		Short:        fmt.Sprintf(
			"[EXPERIMENTAL] Download certificates shared among control-plane nodes from the %s Secret", 
			kubeadmconstants.KubeadmCertsSecret,
		),
...
func newControlPlanePrepareCertsSubphase() workflow.Phase {
	return workflow.Phase{
		...
		//NB. eventually in future we would like to break down this
		// in sub phases for each cert or add the --csr option
...
func newControlPlanePrepareKubeconfigSubphase() workflow.Phase {
	return workflow.Phase{
		...
		//NB. eventually in future we would like to break down this in sub phases for each kubeconfig
...
func newControlPlanePrepareControlPlaneSubphase() workflow.Phase {
	return workflow.Phase{
		...
		//NB. eventually in future we would like to break down this in sub phases for each component
...
func runControlPlanePrepareKubeconfigPhaseLocal(c workflow.RunData) (err error) {
	data, ok := c.(JoinData)
	if !ok {
		return errors.New("control-plane-prepare phase invoked with an invalid data struct")
	}
	...
	fmt.Printf(
		"[kubeconfig] Using kubeconfig folder %q\n", 
		kubeadmconstants.KubernetesDir,
	)

	// Generate kubeconfig files for controller manager, scheduler
	// and for the admin/kubeadm itself
	...
	err = kubeconfigphase.CreateJoinControlPlaneKubeConfigFiles(
		kubeadmconstants.KubernetesDir, cfg,
	)
	if err != nil {
