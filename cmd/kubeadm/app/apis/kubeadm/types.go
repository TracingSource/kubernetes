...
type InitConfiguration struct {
	...
	// (which can be (un)marshalled separately as well)
	...
	// BootstrapTokens 有2处应用
	// 	1. kube-system/bootstrap-token-${xxx} Secret 资源
	// 	2. kubeadm init 完成后, 生成的 join 命令中, --token 的参数值.
	//
	// 在 cmd/kubeadm/app/util/config/initconfiguration.go -> SetBootstrapTokensDynamicDefaults()
	// 完成填充, 一般就是取2个固定长度的随机数, 然后用点号拼接, 赋值给 Token 成员就行了.
	//
	// BootstrapTokens is respected at `kubeadm init` time
	// and describes a set of Bootstrap Tokens to create.
	...
	// In HA setups, this differs from ClusterConfiguration.ControlPlaneEndpoint
	// in the sense that ControlPlaneEndpoint is the global endpoint for the cluster,
	// which then loadbalances the requests to each individual API server.
	// This configuration object lets you customize what IP/DNS name
	// and port the local API server advertises it's accessible on. 
	// By default, kubeadm tries to auto-detect the IP of the default interface
...
type ClusterConfiguration struct {
	...
	// ComponentConfigs holds internal ComponentConfig struct types known to kubeadm,
	// should long-term only exist in the internal kubeadm API
	...
	// in case of kubernetes version is a CI build 
	// (kubernetes version starts with `ci/` or `ci-cross/`)
	// `gcr.io/kubernetes-ci-images` will be used as a default for
	// control plane components and for kube-proxy,
...
// The ClusterStatus will be stored in the kubeadm-config ConfigMap in the cluster, 
// and then updated by kubeadm when additional control plane instance joins or leaves the cluster.
type ClusterStatus struct {
	metav1.TypeMeta

	// APIEndpoints currently available in the cluster,
	// one for each control plane/api server instance.
...
type NodeRegistrationOptions struct {
	...
	// If this field is unset, i.e. nil, in the `kubeadm init` process
	// it will be defaulted to []v1.Taint{'node-role.kubernetes.io/master=""'}.
	// If you don't want to taint your control-plane node, set this field to an
	// empty slice, i.e. `taints: []` in the YAML file.
	...
	// Flags have higher priority when parsing.
	// These values are local and specific to the node kubeadm is executing on.
	KubeletExtraArgs map[string]string

	// IgnorePreflightErrors provides a slice of pre-flight errors to be ignored
...
// (probably as part of Bootstrap Tokens going GA).
// It should not be staged under the kubeadm API as it is now.
