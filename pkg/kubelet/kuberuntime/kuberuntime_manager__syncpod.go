...
func (m *kubeGenericRuntimeManager) SyncPod(
	...
	klog.V(3).Infof(
		...
		podContainerChanges, format.Pod(pod),
	)
	...
	// The list of podIPs is used to generate the hosts file.
	//
	// We default to the IPs in the passed-in pod status,
	// and overwrite them if the sandbox needs to be (re)started.
	var podIPs []string
	if podStatus != nil {
	...
	// 创建一个可以用于启动容器的通用函数, 可以分别启动不同类型的 container.
	// 除了常规的 container, 还有初始化容器 initContainers, 和临时容器 ephemeralContainers.
	// 下面会分别按照这3个种类, 调用该函数依次启动不同的容器.
	//
	// Helper containing boilerplate common to starting all types of containers.
	// typeName is a label used to describe this type of container in log messages,
	// currently: "container", "init container" or "ephemeral container"
	start := func(typeName string, container *v1.Container) error {
		startContainerResult := kubecontainer.NewSyncResult(
			kubecontainer.StartContainer, container.Name,
		)
		...
		if isInBackOff {
			...
			klog.V(4).Infof(
				"Backing Off restarting %v %+v in pod %v", 
				typeName, container, format.Pod(pod),
			)
			return err
		}
		...
		msg, err = m.startContainer(
			podSandboxID, podSandboxConfig, container, pod, podStatus, pullSecrets, podIP, podIPs,
		)
		if err != nil {
			startContainerResult.Fail(err, msg)
			// known errors that are logged in other places are logged at higher levels
			// here to avoid repetitive log spam
			switch {
			case err == images.ErrImagePullBackOff:
				klog.V(3).Infof("%v start failed: %v: %s", typeName, err, msg)
			default:
				utilruntime.HandleError(fmt.Errorf(
					"%v start failed: %v: %s", typeName, err, msg,
				))
			}
			return err
		}
