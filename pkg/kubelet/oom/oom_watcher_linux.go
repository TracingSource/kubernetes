...
// realWatcher 监听系统中的 OOM 事件(通过解析 /dev/kmsg 内容实现此功能)
			...
			if event.ContainerName == "/" {
				...
				if event.ProcessName != "" && event.Pid != 0 {
					eventMsg = fmt.Sprintf(
						...
						eventMsg, event.ProcessName, event.Pid,
					)
				}
				ow.recorder.PastEventf(
					ref, metav1.Time{Time: event.TimeOfDeath}, 
					v1.EventTypeWarning, systemOOMEvent, eventMsg,
				)
			}
		}
