...
// caller:
// 	1. pkg/proxy/iptables/proxier.go -> NewProxier() 作为 proxier.syncRunner() 定时调用
// 	2. pkg/proxy/iptables/proxier.go -> NewProxier() KUBE-PROXY-CANARY
// 	3. pkg/proxy/iptables/proxier.go -> Proxier.OnServiceSynced() kube-proxy 启动时被调用一次
// 	4. pkg/proxy/iptables/proxier.go -> Proxier.OnEndpointsSynced()
// 	5. pkg/proxy/iptables/proxier.go -> Proxier.OnEndpointSlicesSynced()
// 	6. pkg/proxy/iptables/proxier.go -> Proxier.OnNodeAdd()
// 	7. pkg/proxy/iptables/proxier.go -> Proxier.OnNodeUpdate()
// 	8. pkg/proxy/iptables/proxier.go -> Proxier.OnNodeDelete()
//
...
func (proxier *Proxier) syncProxyRules() {
	...
	// 这里假设当被调用时, 确实发生了变更, 由主调函数去过滤未变更的场景.
	//
	...
	for _, svcPortName := range endpointUpdateResult.StaleServiceNames {
		if svcInfo, ok := proxier.serviceMap[svcPortName]; ok &&
			svcInfo != nil && svcInfo.Protocol() == v1.ProtocolUDP {
			klog.V(2).Infof(
				"Stale udp service %v -> %s",
				svcPortName, svcInfo.ClusterIP().String(),
			)
	...
	err := proxier.iptables.SaveInto(
		utiliptables.TableFilter, proxier.existingFilterChainsData,
	)
	if err != nil { // if we failed to get any rules
		klog.Errorf("Failed to execute iptables-save, syncing all rules: %v", err)
	} else { // otherwise parse the output
		existingFilterChains = utiliptables.GetChainLines(
			utiliptables.TableFilter, proxier.existingFilterChainsData.Bytes(),
		)
	}
	...
	if err != nil { // if we failed to get any rules
		...
		existingNATChains = utiliptables.GetChainLines(
			utiliptables.TableNAT, proxier.iptablesData.Bytes(),
		)
	}
	...
	for svcName, svc := range proxier.serviceMap {
		...
		for i, endpointChain := range endpointChains {
			...
			args = append(
				args, "-m", protocol, "-p", protocol,
				"-j", "DNAT", "--to-destination", endpoints[i].Endpoint,
			)
			writeLine(proxier.natRules, args...)
		}
	...
	for chain := range existingNATChains {
		if !activeNATChains[chain] {
			...
			if !strings.HasPrefix(chainString, "KUBE-SVC-") &&
				!strings.HasPrefix(chainString, "KUBE-SEP-") &&
				!strings.HasPrefix(chainString, "KUBE-FW-") &&
				!strings.HasPrefix(chainString, "KUBE-XLB-") {
				// Ignore chains that aren't ours.
				continue
			}
			// We must (as per iptables) write a chain-line for it, which has
			// the nice effect of flushing the chain.
			// Then we can remove the chain.
			writeBytesLine(proxier.natChains, existingNATChains[chain])
			writeLine(proxier.natRules, "-X", chainString)
		}
	}

	// Finally, tail-call to the nodeports chain.
	// This needs to be after all other service portal rules.
	addresses, err := utilproxy.GetNodeAddresses(proxier.nodePortAddresses, proxier.networkInterfacer)
	if err != nil {
		...
		for address := range addresses {
			// TODO(thockin, m1093782566): If/when we have dual-stack support
			// we will want to distinguish v4 from v6 zero-CIDRs.
			if utilproxy.IsZeroCIDR(address) {
				args = append(args[:0],
					"-A", string(kubeServicesChain),
