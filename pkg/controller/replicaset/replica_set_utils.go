...
//
// updateReplicaSetStatus attempts to update the Status.Replicas of the given ReplicaSet,
// with a single GET/PUT retry.
func updateReplicaSetStatus(
	c appsclient.ReplicaSetInterface, rs *apps.ReplicaSet, newStatus apps.ReplicaSetStatus,
) (*apps.ReplicaSet, error) {
	// 如果此if条件满足, 表示已经处于稳定状态, 不需要再做额外操作, 可以直接返回.
	//
	// This is the steady state. It happens when the ReplicaSet doesn't have any expectations,
...
func calculateStatus(
	rs *apps.ReplicaSet, filteredPods []*v1.Pod, manageReplicasErr error,
...
func NewReplicaSetCondition(
	condType apps.ReplicaSetConditionType, status v1.ConditionStatus, reason, msg string,
...
func GetCondition(
	status apps.ReplicaSetStatus, condType apps.ReplicaSetConditionType,
) *apps.ReplicaSetCondition {
