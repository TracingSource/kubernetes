...
func (rsc *ReplicaSetController) manageReplicas(
	...
	if diff < 0 {
		...
		// For example a project with a low quota 
		// that attempts to create a large number of pods will be
...
func (rsc *ReplicaSetController) syncReplicaSet(key string) error {
	...
	updatedRS, err := updateReplicaSetStatus(
		rsc.kubeClient.AppsV1().ReplicaSets(rs.Namespace), rs, newStatus,
	)
	if err != nil {
